...
public final class RPNInterpreter {
    ...
    private BigDecimal binaryOperation(BinaryMathOperation operation) {
        ...
        // Attempting to get two numbers from the stack
        for (int i = 0; i < 2; i++) {
    ...
    /**
     * Parses a binary operation and pushes the result if the binary operation
     * exists in binaryOperationMap. If not, it returns false.
     * 
     * @param current
     * @return Whether or not it was a binary operator or not
     */
    private boolean parseBinaryOperator(char current) {
     ...
     * 
     * @param expression
     *            The entire expression to evaluate
     * @param index
     *            The current index to evaluate
    ...
    private int parseNumber(char expression[], int index) {
        if (Character.isDigit(expression[index]) || expression[index] == '.') {
            String subString = new String(expression, index, expression.length - index);
     ...
     */
    private int parseUnaryOperators(char expression[], int oldIndex) {
        if (unaryOperationMap.containsKey(expression[oldIndex])) {
            for (int currentIndex = oldIndex; currentIndex < expression.length; currentIndex++) {
                char currentChar = expression[currentIndex];
                if (currentIndex != oldIndex && Character.isDigit(currentChar)) {
                    return currentIndex - 1;
                } else if (binaryOperationMap.containsKey(currentChar)) {
    ...
    /**
     * Pushes a number, or reports an error if the number attempted to push is
     * null.
     * 
     * @param number
     *            The number to push.
     ...
     * evaluating groups of characters (numbers or stacked unary operators) It
     * evaluates each character: First checks if it's "q", the terminate signal.
     * Then it checks for a number, then checks for unary operators, then binary
     * operators, then whitespace. If none of the above, it reports invalid.
     * 
     * @param expression
    ...
    public BigDecimal parseExpression(String expression) {
        ...
        for (int index = 0; index < expressionCharArray.length && !interpreterError; index++) {
            char current = expressionCharArray[index];
            int unaryIndex = parseUnaryOperators(expressionCharArray, index);
            int numberIndex = parseNumber(expressionCharArray, index);
            ...
            if (current == 'q') {
                ...
                index = unaryIndex;
            } else if (!parseBinaryOperator(current) && !isWhiteSpace(current)) {
    ...
    public void processExpression(String expression) {
        ...
        // Prints the stack if it's bigger than 1 so that the user doesn't
        // forget
    ...
    /**
     * Sends the terminate signal to the interpreter. Can be called from outside
     * the interpreter.
     */
